...
void Level::loadMap(std::string mapName, Graphics &graphics) {
	...
	// Get the width and height of the tile and store it in _tileSize
	int tileWidth;
	int tileHeight;
				...
				if (pObject != NULL) {
					while (pObject) {
						...
						if (pPolyline != NULL) {
							...
							// Put the points in a string stream
							std::stringstream ss;
							ss << pointString;

							// Split the points by space, add to pairs vector
							Utils::split(ss.str(), pairs, ' ');

							// Now that we have each of the pairs, Loop through
							// the list of pairs and split them into Vector2s
							// and store them in our points vector

							for (int i = 0; i < pairs.size(); i++) {
								std::vector<std::string> ps;
								Utils::split(pairs.at(i), ps, ',');

								// Grab X, Y. Create Vector2
								points.push_back(
									Vector2(
										std::stoi(ps.at(0)),
										std::stoi(ps.at(1))
							...
							for (int i = 0; i < points.size(); i += 2) {
								// Create Vector2 pairs for each segment of a polyline
								// Add to _slopes
...
std::vector<Door> Level::checkDoorCollisions(const Rectangle &other) {
	...
	for (int i = 0; i < this->_doorList.size(); i++) {
		bool check = this->_doorList.at(i).collidesWith(other);
